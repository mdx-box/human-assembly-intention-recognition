    ...
    def plot_to_result(self,
        ...
        if withContours:
            ...
            for i, mask in enumerate(annotations):
                ...
                # print(f'annotation:{annotation}, shape of ann:{annotation.shape}') annotation.shape = (480, 640)
                if not retina:
                    ...
                    #这里的annotation是每一个分割出来物体的轮廓图片，shape为(480, 640)
                    # plt.figure()
                    # plt.imshow(annotation)
                    # plt.show()
                    # print(f'annotation.shape = {annotation.shape}')
                 #contours：这是一个 Python 列表，其中的每个元素都是一个轮廓。每个轮廓本身是一个包含坐标点的数组（即一系列轮廓点的坐标）。
                 # 可以通过遍历 contours 列表来访问每个轮廓的点   
                contours, hierarchy = cv2.findContours(annotation, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                # print(f'contours:{len(contours)}')
                for contour in contours:
                    # print(len(contour))
                    contour_all.append(contour)
                # contour_all是所有轮廓点的集合
            cv2.drawContours(temp, contour_all, -1, (255, 255, 255), 2)
#             temp：这是目标图像，轮廓会在这张图像上绘制。通常是一个彩色图像（如 RGB 或 BGR）或灰度图像。在绘制后，temp 图像将显示包含轮廓的效果。

#            contour_all：包含轮廓数据的列表，通常是通过 cv2.findContours 获得的结果。这个参数可以是单个轮廓或轮廓的列表。

#               -1：用于指定绘制哪些轮廓。-1 表示绘制 contour_all 中的所有轮廓。
# 如果指定一个特定的索引（如 0），则只绘制 contour_all 列表中的第一个轮廓。
# (255, 255, 255)：用于指定轮廓的颜色。在这里 (255, 255, 255) 表示白色（在 BGR 模式下为白色）。可以根据需要更改颜色，例如 (0, 0, 255) 表示红色。

# 2：指定轮廓线的粗细。这里设置为 2，表示线条宽度为 2 个像素。可以根据需要更改为其他值。
    ...
    def plot(self,
        ...
        if len(annotations) == 0:
            return None
        result, contouur = self.plot_to_result(
            annotations, 
            bboxes, 
            points, 
            point_label, 
